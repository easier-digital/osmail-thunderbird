...
3. Replace `YOUR_ORG` in `distribution/policies.json` with your GitHub org/username.
4. Place a 64x64 PNG logo at `theme/icons/logo.png`.
5. Host `autoconfig/config-v1.1.xml` at `https://autoconfig.osmail.ca/mail/config-v1.1.xml`.
6. Trigger the first build: `gh workflow run build.yml`
7. After the first release, update `__THEME_XPI_URL__` in `policies.json` with the theme XPI asset URL.

## Deployment via Atera RMM

```powershell
$release = Invoke-RestMethod "https://api.github.com/repos/YOUR_ORG/thunderbird-osmail/releases/latest"
